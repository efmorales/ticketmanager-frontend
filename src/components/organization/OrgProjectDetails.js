...
const ProjectDetails = () => {
  ...
  const sortAndFilterTickets = (tickets) => {
    ...
    if (filterCriteria) {
      sortedTickets = sortedTickets.filter(
        (ticket) => ticket.status === filterCriteria
      );
    }
    ...
    const fetchProject = async () => {
      try {
        const { data } = await api.get(
          `/organizations/${organization._id}/projects/${projectId}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem(TOKEN_KEY)}`,
            },
  ...
  useEffect(() => {
    ...
    setTicketsToShow(
      sortedAndFilteredTickets.slice(firstItemIndex, lastItemIndex)
    );
  ...
  const updateTicket = (updatedTicket, removedTicketId) => {
    if (removedTicketId) {
      setTickets((prevTickets) =>
        prevTickets.filter((ticket) => ticket._id !== removedTicketId)
      );
    } else {
      setTickets((prevTickets) =>
