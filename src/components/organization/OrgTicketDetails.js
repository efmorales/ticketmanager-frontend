    ...
    const fetchProjectMembers = async () => {
      try {
        const { data } = await api.get(
          `/organizations/${organization._id}/projects/${ticket.projectId}/members`,
          {
            headers: {
  ...
  return (
    <div className="ticket-details-container">
      ...
      <label>
        <select
          value={
            (editedData.assignedTo && editedData.assignedTo._id) ||
            (ticket.assignedTo.length > 0 ? ticket.assignedTo[0]._id : "")
          }
